...
abstract class HavcModule extends HavcBase {
	...
	public function settings_section_callback($args){
		do_action('havc_module_settings_section_callback', $args);
	}
	...
	protected function render_setting_field($args){
		...
		if(!$field['type']){
			echo __('Error: Setting field type is empty!', HAVC_SLUG);
			return;
		}

		if(!$field['name']){
			echo __('Error: Setting field name is empty!', HAVC_SLUG);
			return;
		}
		...
		switch ($field['type']) {
			...
			default:
					do_action('havc_module_render_setting_field', $field);
				break;
		}
